...
def run_config(config_path: Path,
    ...
    if enable_nvidia_smi:
        time.sleep(5)  # 추가된 디버깅 시간
        nvidia_smi_output_file = output_dir / Path("nvidia_smi_{}.csv".format(run_id))
        if nvidia_smi_output_file.is_file() and nvidia_smi_output_file.stat().st_size == 0:
            print(f"Error: {nvidia_smi_output_file} is empty.")
        else:
            print(f"nvidia-smi output file size: {nvidia_smi_output_file.stat().st_size} bytes")

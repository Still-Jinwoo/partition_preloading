...
def get_results_summary_(experiment_dir: Path):
    ...
    for i in range(num_runs):
        ...
        # print("This is the result PATH: ", experiment_dir)
        ...
        if (experiment_dir / Path("dstat_{}.csv".format(i))).exists():
            ...
            if not dstat_df.empty:
                if "CPU User Utilization" in dstat_df.columns and "CPU Sys Utilization" in dstat_df.columns:
                ...
                if "Bytes Read" in dstat_df.columns and "Bytes Written" in dstat_df.columns:
                ...
                if "Memory Used" in dstat_df.columns:
                    result.peak_memory_usage = dstat_df["Memory Used"].max()
            else:
                print(f"Warning: {experiment_dir / Path('dstat_{}.csv'.format(i))} is empty or missing columns.")

        nvidia_smi_file = experiment_dir / Path("nvidia_smi_{}.csv".format(i))
        if nvidia_smi_file.exists():
            if nvidia_smi_file.stat().st_size > 0:
                nvidia_smi_df = parse_result.parse_nvidia_smi(nvidia_smi_file)
                result.mean_gpu_util = nvidia_smi_df["GPU Compute Utilization"].mean()
                result.std_gpu_util = nvidia_smi_df["GPU Compute Utilization"].std()
            else:
                print(f"Warning: {nvidia_smi_file} is empty.")
        else:
            print(f"Warning: {nvidia_smi_file} does not exist.")

...
std::vector<NodeId> load_next_batch(int batch_size) {
    std::vector<NodeId> batch = get_next_batch_nodes();
    
    // Sort nodes by importance before loading
    std::sort(batch.begin(), batch.end(), [](NodeId a, NodeId b) {
        return calculate_importance(a) > calculate_importance(b);
    });

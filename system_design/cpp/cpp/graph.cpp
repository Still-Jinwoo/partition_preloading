...
CeleritasGraph::CeleritasGraph() {};

CeleritasGraph::CeleritasGraph(EdgeList src_sorted_edges, EdgeList dst_sorted_edges, int64_t num_nodes_in_memory, int num_hash_maps) {
...
Indices CeleritasGraph::getNodeIDs() {
    return node_ids_;
}

Indices CeleritasGraph::getEdges(bool incoming) {
    if (incoming) {
...
Indices CeleritasGraph::getRelationIDs(bool incoming) {
...
Indices CeleritasGraph::getNeighborOffsets(bool incoming) {
    if (incoming) {
        return in_offsets_;
    } else {
        return out_offsets_;
...
Indices CeleritasGraph::getNumNeighbors(bool incoming) {
    if (incoming) {
...
void CeleritasGraph::to(torch::Device device) {
...
// 1 hop sampler
std::tuple<torch::Tensor, torch::Tensor> CeleritasGraph::getNeighborsForNodeIds(torch::Tensor node_ids, bool incoming, NeighborSamplingLayer neighbor_sampling_layer, int max_neighbors_size, float rate) {
    //std::cout << "@@@ In graph.cpp get Neighbors For Node ids @@@ \n";
    int gpu = 0;
